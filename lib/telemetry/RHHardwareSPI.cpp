...
//      SPI.setClockSpeed(1, MHZ);
  ...
  uint8_t dataMode;
  if (_dataMode == DataMode0)
    dataMode = 0; // SPI Mode 0
  else if (_dataMode == DataMode1)
    dataMode = 1; // SPI Mode 1
  else if (_dataMode == DataMode2)
    dataMode = 2; // SPI Mode 2
  else if (_dataMode == DataMode3)
    dataMode = 3; // SPI Mode 3

  // WiringPi does not directly support changing bit order after initialization, 
  // so we assume MSB first as WiringPi's SPI is MSB by default.
  ...
  SPI.begin(0, frequency, dataMode);
#else
